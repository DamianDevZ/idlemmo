-- Make the S/A/B/C/D/F damage multipliers configurable via game_config.
-- Previously hardcoded as CASE WHEN 'S' THEN 1.5 ... in the arena function.
--
-- Changes:
--   1. Insert grade_mult_s/a/b/c/d/f rows into game_config
--   2. Rewrite join_arena_queue to SELECT those values instead of hardcoding them

-- ─── 1. Seed grade multiplier config rows ────────────────────────────────────
INSERT INTO game_config (key, value, default_value, min_value, max_value, category, label, description)
VALUES
  ('grade_mult_s', 1.5, 1.5, 1.0, 5.0, 'combat', 'Grade S Multiplier', 'Multiplier applied to the stat-bonus portion of damage for S-grade weapons'),
  ('grade_mult_a', 1.4, 1.4, 1.0, 5.0, 'combat', 'Grade A Multiplier', 'Multiplier applied to the stat-bonus portion of damage for A-grade weapons'),
  ('grade_mult_b', 1.3, 1.3, 1.0, 5.0, 'combat', 'Grade B Multiplier', 'Multiplier applied to the stat-bonus portion of damage for B-grade weapons'),
  ('grade_mult_c', 1.2, 1.2, 1.0, 5.0, 'combat', 'Grade C Multiplier', 'Multiplier applied to the stat-bonus portion of damage for C-grade weapons'),
  ('grade_mult_d', 1.1, 1.1, 1.0, 5.0, 'combat', 'Grade D Multiplier', 'Multiplier applied to the stat-bonus portion of damage for D-grade weapons'),
  ('grade_mult_f', 1.0, 1.0, 1.0, 5.0, 'combat', 'Grade F Multiplier', 'Multiplier applied to the stat-bonus portion of damage for F-grade weapons (baseline)')
ON CONFLICT (key) DO NOTHING;

-- ─── 2. Rewrite join_arena_queue to read multipliers from game_config ─────────
...
DECLARE
  ...
  -- Grade multipliers (loaded from game_config, editable via admin)
  v_grade_mult_s      numeric;
  v_grade_mult_a      numeric;
  v_grade_mult_b      numeric;
  v_grade_mult_c      numeric;
  v_grade_mult_d      numeric;
  v_grade_mult_f      numeric;
...
BEGIN
  ...
  -- Grade damage multipliers (editable via admin → Grading System)
  SELECT COALESCE(value, 1.5) INTO v_grade_mult_s FROM game_config WHERE key = 'grade_mult_s';
  SELECT COALESCE(value, 1.4) INTO v_grade_mult_a FROM game_config WHERE key = 'grade_mult_a';
  SELECT COALESCE(value, 1.3) INTO v_grade_mult_b FROM game_config WHERE key = 'grade_mult_b';
  SELECT COALESCE(value, 1.2) INTO v_grade_mult_c FROM game_config WHERE key = 'grade_mult_c';
  SELECT COALESCE(value, 1.1) INTO v_grade_mult_d FROM game_config WHERE key = 'grade_mult_d';
  SELECT COALESCE(value, 1.0) INTO v_grade_mult_f FROM game_config WHERE key = 'grade_mult_f';
  ...
  -- Grade multipliers: read from game_config (editable in admin → Grading System)
  v_char_grade_mult := CASE v_char_scale_grade
    WHEN 'S' THEN v_grade_mult_s WHEN 'A' THEN v_grade_mult_a WHEN 'B' THEN v_grade_mult_b
    WHEN 'C' THEN v_grade_mult_c WHEN 'D' THEN v_grade_mult_d ELSE          v_grade_mult_f
  END;
  ...
  -- final_damage = weapon_base_at_tier + round(stat_bonus × grade_mult), min 1
  v_char_stat_bonus :=
