-- Fix two things from migration 022:
-- 1. Remove DEX from combat ordering entirely — DEX is a damage scaling stat,
--    not an initiative stat. Calling character's attack is always logged first.
-- 2. Tiebreaker: when both fighters die in the same round, whoever ended up
--    further below zero HP loses (they absorbed the more decisive final blow).
--    e.g. char_raw=-5, opp_raw=-7 → opp dies.
...
DECLARE
  ...
  -- Raw unclamped HP after each round (used for overkill tiebreaker)
...
BEGIN
  ...
  -- Initialise raw HP to positive so tiebreaker check is correct before first round
  ...
  WHILE v_char_hp > 0 AND v_opp_hp > 0 AND v_round <= v_max_rounds LOOP
    ...
    -- Track raw (unclamped) values to measure overkill depth
    v_opp_hp_raw  := v_opp_hp  - v_char_net;
    v_char_hp_raw := v_char_hp - v_opp_net;

    -- Calling character's attack always logged first (cosmetic display order)
