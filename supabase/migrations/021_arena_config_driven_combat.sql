-- Two fixes:
-- 1. All combat constants now read from game_config (nothing hardcoded).
--    Adds unarmed_base_damage and end_armor_factor config rows.
-- 2. Fixes NULL bug: PostgreSQL SELECT INTO sets variables to NULL when no row
--    is found, ignoring any DECLARE-time defaults. Added explicit COALESCE
--    after each gear query so unarmed/unarmored fighters get correct fallbacks.

INSERT INTO game_config (category, sort_order, key, label, description, value, default_value, min_value, max_value, step, unit)
VALUES
  ('combat_damage', 5, 'unarmed_base_damage', 'Unarmed Base Damage',
   'Base damage dealt per hit when no weapon is equipped.',
   25, 25, 1, 500, 1, null),
  ('combat_damage', 6, 'end_armor_factor', 'END → Armor Factor',
   'Flat armor points granted per point of Endurance. total_armor = end * factor + item_base_defense.',
   5, 5, 0, 50, 1, null)
...
AS $$
DECLARE
  -- game_config constants (loaded at runtime, never hardcoded)
  v_base_hp           numeric;
  ...
  -- per-strike locals
  v_attacker          text;  v_defender        text;
  v_raw               float; v_deflect_pct     float;
  v_deflected_amt     float; v_net             float;
  v_atk_hp            float; v_def_hp_before   float;
...
BEGIN
  ...
  -- FOR UPDATE SKIP LOCKED: only one concurrent transaction wins the opponent
  ...
  -- Explicit COALESCE after SELECT INTO: no rows → variables become NULL
  ...
  -- ── Derived combat stats (all constants from game_config) ─────────────
  ...
  -- damage = weapon_base * (1 + STR / str_melee_divisor)
  ...
  -- deflect = total_armor / (total_armor + armor_divisor)
  -- total_armor = end * end_armor_factor + item_base_defense
