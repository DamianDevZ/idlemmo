-- Fix two bugs:
-- 1. Race condition: two concurrent join_arena_queue calls could both find
--    each other and run independent simulations → FOR UPDATE SKIP LOCKED
-- 2. Max HP inconsistency: checkArenaMatch was re-querying character_attributes
--    and could get different values → store player1_max_hp/player2_max_hp in
--    arena_matches so both players read the exact same numbers

ALTER TABLE arena_matches
  ADD COLUMN IF NOT EXISTS player1_max_hp int,
  ADD COLUMN IF NOT EXISTS player2_max_hp int;
...
begin
  ...
  -- FOR UPDATE SKIP LOCKED: only one concurrent transaction can claim an opponent.
  -- The losing transaction sees no rows, returns matched:false, and keeps polling.
