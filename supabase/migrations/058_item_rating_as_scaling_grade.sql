-- Use the per-instance item_rating (from character_inventory) as the weapon
-- scaling grade instead of the fixed primary_scaling_grade on item_definitions.
--
-- Rationale: grade already represents quality on every dropped item. Having a
-- separate fixed grade on the item definition is redundant and confusing — an
-- iron sword that drops as S should naturally scale better with STR than one
-- that dropped as F.
--
-- Changes:
--   1. Rewrite join_arena_queue to use ci.item_rating instead of id.primary_scaling_grade
--   2. Drop primary_scaling_grade and secondary_scaling_grade from item_definitions

-- ─── 1. Drop now-redundant columns ───────────────────────────────────────────
ALTER TABLE item_definitions
  DROP COLUMN IF EXISTS primary_scaling_grade,
  DROP COLUMN IF EXISTS secondary_scaling_grade;

-- ─── 2. Rewrite join_arena_queue ─────────────────────────────────────────────
...
BEGIN
  ...
  -- Scaling grade now comes from the inventory item_rating (the quality of this
  -- specific dropped instance), not a fixed field on the item definition.
