...
export function ArenaQueueButton({ characterId, isQueued: initialQueued }: Props) {
  ...
  // ── Wall-clock ticker: drives countdown + strike reveals from combatStartsAt ─
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      // Combat started — clear countdown display
      setCountdown(null);
  ...
  // ── Auto-scroll log to latest strike ───────────────────────────────────────
  useEffect(() => {
  ...
  // ── Fighting: countdown + live combat in one phase ──────────────────────────
  if (phase === 'fighting' && combatData) {
    // Countdown screen — combatStartsAt hasn't arrived yet
    if (countdown !== null) {
      return (
